...
<div id="alert" class="fixed flex gap-4 top-10 right-6 p-4 rounded-lg max-w-xs w-full transition duration-200 ease-in-out" role="alert" x-data="{ showFlash: true }" :class="{
    'active' : showFlash, 
    '' : !showFlash,
    'alert-danger dark:alert-dark' : typeStatus == true,
    'alert-success dark:alert-dark' : typeStatus == false,
    }" x-init="setTimeout(() => showFlash = false, 3000)">

    <span x-text="console.log(typeStatus)"></span>
