            ...
            <template x-if="!isLoadingArticle">
                <div class="container mx-auto flex flex-wrap lg:flex-nowrap gap-6 md:gap-4">
                    ...
                    <div class="col md:mx-0 col-12 lg:col-8">
                        ...
                        <div
                            class="px-4 py-5 rounded-primary bg-white dark:bg-slate-secondary shadow-[0px_0px_4px_rgba(0,0,0,0.25)]">

                            <h2 class="text-md text-[#3A3440] dark:text-white font-bold border-b border-gray-third dark:border-gray-secondary mb-4 pb-2"
                                x-text="content ? content?.title : detailArticle?.title">Judul Artikel
                            </h2>
                            ...
                            <div class="flex items-start flex-wrap gap-4 md:gap-0 justify-between">
                                <div class="flex gap-3">
                                    <figure>
                                        <div>
                                            <img x-bind:src="detailArticle?.author?.photo != null && detailArticle?.author?.photo != '' ?
                                                imgUrl + detailArticle?.author?.photo :
                                                imgUrl + 'img/user1.png'"
                                                src=""
                                                class="w-[50px] h-[50px] bg-gray-secondary rounded-full border-none"
                                                alt="">
                                        </div>
                                    </figure>
                                    <div class="">
                                        <b class="text-base font-semibold" x-text="detailArticle?.author?.username">Nama
                                            Author</b>
                                        <span class="block text-gray-primary dark:text-gray-third"
                                            x-text="detailArticle?.author?.email"></span>
                                    </div>
                                </div>
                                <div x-show="!isLoadingArticle" class="flex items-center gap-3 translate-y-1"
                                    x-data="helpers">
                                    ...
                                    <span class="flex items-center gap-1">
                                        ...
                                        <p>
                                            <span
                                                x-show="detailArticle?.total_views_sum == null ? detailArticle.total_views_sum = 0 : ''"></span>
                                            <span
                                                x-text="content ? content?.total_views + ' views' : detailArticle?.total_views_sum + ' views'">
                                            </span>
                                        </p>
                                    </span>
                                </div>
                            ...
                            <div class="mt-4">
                                <figure class="mt-4">
                                    <img x-bind:src="content ? imgUrl + content?.thumbnail : imgUrl + detailArticle?.thumbnail"
                                        src="" class="w-full h-[345px] bg-gray-secondary rounded-primary"
                                        alt="">
                                </figure>
                                <p x-show="!isLoadingArticle"
                                    class="mt-6 font-quickSand text-[#3A3440] dark:text-white font-semibold"
                                    x-html="content ? content?.description : detailArticle?.description">
                                </p>
                            </div>

                            <div class="flex items-center w-full"
                                x-bind:class="detailArticle?.tags.length > 0 ? 'justify-between mt-12' : 'justify-end mt-6'">
                                <div class="flex content-center flex-wrap gap-3"
                                    x-bind:class="detailArticle?.tags.length > 0 ? '' : 'hidden'">
                                ...
                                <template x-if="detailArticle?.subarticles.length > 0">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div class="flex items-center gap-2">
                                            ...
                                            <button
                                                x-show="content?.id != detailArticle?.subarticles?.[detailArticle?.subarticles?.length - 1]?.id"
                                                x-on:click="
                                                                if(content){
                                                                    id = content.id;
                                                                    id = detailArticle?.subarticles?.findIndex(x => x.id == id);
                ...
                <div class="col md:mx-0 col-12 lg:col-4">
                    ...
                    <div class="px-5 py-6 bg-white dark:bg-slate-secondary shadow-[0px_0px_4px_rgba(0,0,0,0.3)] rounded-lg"
                        x-data="helpers">

                        <ul
                            class="relative left-1/2 -translate-x-1/2 flex items-center justify-center gap-4 dark:shadow-[0px_0px_4px_#fff] w-max p-2 rounded-lg">
                        ...
                        <template x-if="detailArticle?.subarticles.length > 0">
                            <div x-data="{
                                ...
                                <div class="flex items-center w-full gap-2 mt-5 mb-3">
                                    <template
                                        x-if="detailArticle?.type == 'free' || detailArticle?.subarticles?.length >= 3">
                                        <button type="button"
                                            x-bind:class="detailArticle?.subarticles.filter(item => item.type == 'free').length > 0 &&
                                                detailArticle?.subarticles.filter(item => item.type == 'paid').length ==
                                                0 ? 'active' : ''"
                                            class="p-2 flex-1 rounded-pill font-semibold font-iceberg border border-primary hover:bg-primary hover:text-white dark:text-white dark:border-white dark:hover:bg-slate-third dark:hover:opacity-80 transition duration-200 ease-in-out"
                                            @click="
                                                            type = type == 'paid' ? '' : 'paid';
                                    ...
                                    <template
                                        x-if="detailArticle?.type == 'paid' && detailArticle?.subarticles?.length != 0">
                                        <button type="button"
                                            x-bind:class="detailArticle?.subarticles.filter(item => item.type == 'paid').length > 0 &&
                                                detailArticle?.subarticles.filter(item => item.type == 'free').length ==
                                                0 ? 'active' : ''"
                                            class="p-2 flex-1 rounded-pill font-semibold font-iceberg border border-primary hover:bg-primary hover:text-white dark:text-white dark:border-white dark:hover:bg-slate-third dark:hover:opacity-80 transition duration-200 ease-in-out"
                                            @click="
                                                        type = type == 'free' ? '' : 'free';
                                ...
                                <p x-ref="statusUser" id="alert"
                                    class="hidden w-full p-3 mt-5 mb-3 rounded-primary border border-primary dark:border-white dark:bg-slate-primary"
                                    x-bind:class="detailArticle?.subarticles.length > 0 ? 'mt-4' : ''">
                                    You have to
                                    <a href="{{ route(' transaction.create') }}"
                                        class="span hover:text-opacity-80 dark:hover:text-opacity-80 dark:text-white font-bold transition duration-200 ease-in-out">Subscribe</a>
                                    to Access this
                                </p>
                                ...
                                <div id="wrapperSub" class="block max-h-[408px] overflow-y-auto has-scrollbar">
                                    <ul class="flex flex-col gap-4 pr-4" x-transition
                                        x-bind:class="detailArticle?.subarticles.length > 0 ? 'mt-2' : ''">
                                        <template x-for="(item, index) in detailArticle?.subarticles">
                                            <li x-show="item.type != type"
                                                @click="
                                                ...
                                                :class="{
                                                    ...
                                                    'bg-white border-slate-primary text-slate-primary': content?.id ==
                                                        item.id
                                                }"
                                                ...
                                                <a class="text-base lg:text-md font-iceberg">
                                                    ...
                                                    {{-- <b x-show="data_user?.subscribe_status != 1 && item.type == 'paid'"
                                                        x-text="'(' + item.type.toUpperCase() + ')'"></b> --}}
                                                </a>
                                                <template x-if="content?.id == item.id">
                                                    <p class="flex items-center gap-1">
                                                        <span
                                                            class="w-4 h-4 rounded-full bg-slate-primary dark:bg-slate-primary"></span>
                                                        <b>Active</b>
                                                    </p>
                                                </template>
                                        ...
                                        <template x-if="back">
                                            <li @click="content = null; back = false;" {{-- @click="window.location.reload();" --}}
                                                ...
                                                <button
                                                    class="text-base lg:text-md font-iceberg hover:text-opacity-80">Back to
                                                    Article</button>
                                            </li>
                                        </template>
                                    </ul>
