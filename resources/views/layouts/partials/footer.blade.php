<footer class="py-8 dark:text-white">
<div class="container mx-auto flex flex-col items-center">

    <h1 class="text-[18px] lg:text-md text-center">
        <span class="span dark:text-slate-third font-iceberg text-[26px]">@Freemium</span> 
        2022. All Right Copyright Reserved</h1>

    <p class="my-6 font-iceberg text-[18px] lg:text-md text-center">Need a Help? Email: freemium.app@gmail.com</p>

    <ul class="flex items-center gap-4">
        <li class="p-3 flex items-center justify-center rounded-full outline outline-1 outline-primary dark:outline-white hover:bg-primary dark:hover:bg-slate-secondary hover:text-white transition duration-200 ease-in-out">
            <a href="">
                <i data-feather="facebook"></i>
            </a>
        </li>
        <li class="p-3 flex items-center justify-center rounded-full outline outline-1 outline-primary dark:outline-white hover:bg-primary dark:hover:bg-slate-secondary hover:text-white transition duration-200 ease-in-out">
            <a href="">
                <i data-feather="twitter"></i>
            </a>
        </li>
        <li class="p-3 flex items-center justify-center rounded-full outline outline-1 outline-primary dark:outline-white hover:bg-primary dark:hover:bg-slate-secondary hover:text-white transition duration-200 ease-in-out">
            <a href="">
                <i data-feather="instagram"></i>
            </a>
        </li>
    </ul>

</div>
<script>
    feather.replace()
</script>
</footer>